...
class PredictionErrorType(Enum):
    ...
    POSITIVE = "positive"   # Better than expected → reinforce
    NEGATIVE = "negative"   # Worse than expected → adjust
    NEUTRAL = "neutral"     # As expected → maintain
...
class VentralStriatum:
    ...
    def __init__(
        ...
        print(f"[VS] Initializing Ventral Striatum...")
        ...
        print(f"[VS] Critic ready. Weights: {self.WEIGHTS}")
        print(f"[VS] LLM reflection: {'enabled' if use_llm_reflection else 'disabled'}")
    ...
    def compute_actual_utility(self, signals: OutcomeSignals) -> Tuple[float, Dict[str, float]]:
        ...
        goal_progress = -signals.goal_distance_delta
        goal_progress = max(-1.0, min(1.0, goal_progress))
        components["goal_progress"] = goal_progress * self.WEIGHTS["goal_progress"]
        
        replan_penalty = min(1.0, signals.replanning_count * 0.2)
...
- Threat before/after: {signals.threat_level_before:.2f} → {signals.threat_level_after:.2f}
- Confidence before/after: {signals.confidence_before:.2f} → {signals.confidence_after:.2f}
...
Provide a brief 1-2 sentence reflection on why the prediction differed from reality."""
    ...
    def _generate_learning_signal(
        ...
        if error_type == PredictionErrorType.POSITIVE:
            ...
            return (
                f"✨ BETTER THAN EXPECTED (+{error:.2f}): "
    ...
    def _print_critique(self, feedback: CriticFeedback):
        ...
        for pe in feedback.prediction_errors:
            rpe_icon = "🟢" if pe.error_type == PredictionErrorType.POSITIVE else \
                      "🔴" if pe.error_type == PredictionErrorType.NEGATIVE else "⚪"
        ...
        if feedback.bias_detected:
            print(f"⚠️  BIAS: {feedback.bias_detected}")
        
        if feedback.component_biases:
            print(f"📊 Component biases: {feedback.component_biases}")
    ...
    def apply_feedback_to_ofc(self, ofc: OFC, feedback: CriticFeedback):
        ...
        if da_adj != 0 or st_adj != 0:
            ...
            print(f"[VS→OFC] Applied: DA={new_da:.2f}, 5-HT={new_st:.2f}")
...
if __name__ == "__main__":
    print("\nVentral Striatum - Signal-based critic demo")
